...
HRESULT APIENTRY hkPresent(IDXGISwapChain3* pSwapChain, UINT SyncInterval, UINT Flags) {
	if (!ImGui_Initialised) {
		if (SUCCEEDED(pSwapChain->GetDevice(__uuidof(ID3D12Device), (void**)&DirectX12Interface::Device))) {
			...
			if (!io.Fonts->AddFontFromFileTTF((const char*)u8"OPPO Sans 4.0.ttf", 20.f, NULL, io.Fonts->GetGlyphRangesChineseFull()))
				io.Fonts->AddFontFromFileTTF("C:/Windows/Fonts/SimHei.ttf", 17.0f, NULL, io.Fonts->GetGlyphRangesChineseFull());
			else
				io.Fonts->AddFontFromFileTTF((const char*)u8"OPPO Sans 4.0.ttf", 20.f, NULL, io.Fonts->GetGlyphRangesChineseFull());
	...
	{
	case 0:
		Config.ConfigUIColorBule(style, colors);
		break;
	case 1:
	...
	if (GetAsyncKeyState(FPSUnlock.ShowMenu) & 1)
		ShowMenu = !ShowMenu;
...
HMODULE GetModuleBaseByName(const char* moduleName) {
	HMODULE hModule = GetModuleHandleA(moduleName);

	if (hModule == NULL) {
		std::cout << "获取模块 \"" << moduleName << "\" 句柄失败，错误码: " << GetLastError() << std::endl;
	}
...
BOOL APIENTRY DllMain(HMODULE hModule, DWORD dwReason, LPVOID lpReserved) {
	...
	case DLL_PROCESS_ATTACH:
		...
		ExeModule = GetModuleBaseByName("SB-Win64-Shipping.exe");
		break;
